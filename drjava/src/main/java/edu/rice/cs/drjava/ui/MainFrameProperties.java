...
public class MainFrameProperties {
    ...
    private MainFrame objFrame;

    public MainFrameProperties(MainFrame.PositionListener posListener, AbstractGlobalModel _model, MainFrame objFrame){
        _posListener = posListener;
        this._model = _model;
    ...
    public void setUpDrJavaProperties() {
        ...
        PropertyMaps.TEMPLATE.setProperty("Misc", getMiscProperty());
        ...
        PropertyMaps.TEMPLATE.setProperty("Action", new DrJavaActionProperty("action.save.all", "(Save All...)", "Execute a \"Save All\" action.") {
            public void update(PropertyMaps pm) { objFrame._saveAll(); }
            public boolean isCurrent() { return false; }
        });
        ...
        PropertyMaps.TEMPLATE.setProperty("Action", new DrJavaActionProperty("action.compile.all", "(Compile All...)","Execute a \"Compile All\" action.") {
            public void update(PropertyMaps pm) { objFrame._compileAll(); }
            public boolean isCurrent() { return false; }
        });
    ...
    private DrJavaProperty getMiscProperty() {
        String help = "Get an input string from the user.\n" +
                "Optional attributes:\n" +
                "\tprompt=\"<prompt to display>\"\n" +
                "\tdefault=\"<suggestion to the user>\"";
        DrJavaProperty miscProperty = new DrJavaProperty("input", "(User Input...)",             help)
        {
            ...
            public void update(PropertyMaps pm) {
                ...
                if (input == null) input = "";
                input = JOptionPane.showInputDialog(objFrame, msg, input);
        ...
        DrJavaActionProperty actionProperty = new DrJavaActionProperty("action.open.file", "(Open File...)","Execute an \"Open File\" action.\n"+"Required attributes:\n"+"\tfile=\"<file to open>\"\n"+"Optional attributes:\n"+"\tline=\"<line number to display>") {
            public void update(PropertyMaps pm) {
                if (_attributes.get("file") != null) {
                    ...
                    if (lineNo >= 0) {
                        final int l = lineNo;
                        Utilities.invokeLater(new Runnable() { public void run() { objFrame._jumpToLine(l); } });
                    }
                }
            }
            /** Reset the attributes. */
